# Hi there! 👋 I'm Christian Chukwu

🎓 Software Development Student at **BYU–Idaho**

I'm a passionate software developer who enjoys creating practical, user-friendly web applications that solve real-world problems. I have experience building responsive websites, interactive web applications, and database-driven projects while continuously expanding my knowledge in software engineering, JavaScript, React, and cybersecurity.

I enjoy learning new technologies, collaborating on projects, and challenging myself with problems that improve both my technical and creative thinking. My goal is to become a professional software engineer who builds applications that make a meaningful impact.

---

## 💻 Technical Skills

### Languages
- JavaScript
- Python
- Java
- HTML5
- CSS3
- SQL

### Frameworks & Libraries
- React
- Node.js
- Express.js

### Tools & Technologies
- Git
- GitHub
- VS Code
- Vite
- npm
- REST APIs
- Responsive Web Design
- JSON
- Local Storage

### Currently Learning
- TypeScript
- Advanced React
- Software Architecture
- Cloud Computing
- Cybersecurity Fundamentals
## 🛠️ Languages & Tools

<div align="left">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" title="JavaScript" width="45" height="45"/>&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" title="HTML5" width="45" height="45"/>&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg" title="CSS3" width="45" height="45"/>&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" title="Python" width="45" height="45"/>&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/java/java-original.svg" title="Java" width="45" height="45"/>&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg" title="React" width="45" height="45"/>&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nodejs/nodejs-original.svg" title="Node.js" width="45" height="45"/>&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg" title="Git" width="45" height="45"/>&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/github/github-original.svg" title="GitHub" width="45" height="45"/>&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vscode/vscode-original.svg" title="VS Code" width="45" height="45"/>&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg" title="MySQL" width="45" height="45"/>&nbsp;
</div>
---

# 🚀 Featured Projects

### 🏢 Abidjan Center of Commerce
Designed and developed a modern business website showcasing commercial opportunities, services, and information for businesses operating in Abidjan.

### 🌍 Bilingua
A bilingual educational platform designed to help learners study both French and English through interactive lessons and practical learning resources.

### 🌤️ Weather Application
Built a weather application that retrieves real-time weather information using an external API and presents it through a clean, responsive interface.

### 🚣 White Water Rafting Website
Developed a complete rafting company website featuring responsive layouts, service information, booking pages, and modern web design principles.

### 🛕 LDS Temple Directory
Created an interactive website that displays temples of The Church of Jesus Christ of Latter-day Saints, allowing users to explore temple information and locations.

### 👨‍⚖️ Latter-day Prophets Website
Built an informational website highlighting prophets of The Church of Jesus Christ of Latter-day Saints, including historical information and biographies.

---

## 📚 What I'm Working On

- Building full-stack web applications
- Improving JavaScript and React development skills
- Learning software design patterns
- Expanding my cybersecurity knowledge
- Developing projects that solve real-world problems

---

## 🌱 Goals

- Contribute to open-source projects
- Build scalable web applications
- Earn industry certifications
- Continue growing as a software engineer
- Collaborate with development teams on meaningful projects

---
## 📊 GitHub Statistics

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=christian-chukwu&show_icons=true&theme=github_dark&hide_border=true" alt="Christian Chukwu's GitHub Stats" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=christian-chukwu&layout=compact&theme=github_dark&hide_border=true" alt="Top Languages" />
</p>

<p align="center">
  <img src="https://streak-stats.demolab.com?user=christian-chukwu&theme=github-dark&hide_border=true" alt="GitHub Streak" />
</p>

## 📫 Connect With Me

## 🌐 Connect with Me

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Christian%20Chukwu-blue?style=for-the-badge&logo=linkedin)](https://www.linkedin.com/in/christian-chukwu-a266201b1)

[![GitHub](https://img.shields.io/badge/GitHub-christian--chukwu-black?style=for-the-badge&logo=github)](https://github.com/christian-chukwu)
---

![](https://komarev.com/ghpvc/?username=christian-chukwu&style=for-the-badge)

> *"Continuous learning, consistent improvement, and building software that makes a difference."*
